...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help)
        cat <<USAGE
Usage: $SCRIPT_NAME [--zones "101,102"] [--timeout SECONDS] [--pause SECONDS]
USAGE
        exit 0
        ;;
      *)
...
print_step "Fetching a recorded asset for playback..."
RECORDS_QUERY='{"type":"RECORDING","page":1,"length":1,"order":[{"column":"created_at","dir":"desc"}],"filter":[]}'
RECORDS_RESPONSE=$(api_post "records/list" "$RECORDS_QUERY")
RECORD=$(echo "$RECORDS_RESPONSE" | jq '.data[0]')
if [[ "$RECORD" == "null" ]]; then
  abort "No recordings are available in the system."
fi

PLAYLIST_ITEM=$(echo "$RECORD" | jq '{
  id: (.id | tostring),
  title: (.name // .title // ("ID " + (.id|tostring))),
  filename: .filename,
  extension: (.extension // (.mime_type | split("/")[-1]) // null),
  path: (.metadata.path // .path // null),
  storage_path: (.metadata.storage_path // .storage_path // null),
  metadata: (.metadata // {})
}')

print_step "Starting central file broadcast..."
START_PAYLOAD=$(jq -n --argjson zones "$ZONES_JSON" --argjson item "$PLAYLIST_ITEM" '{
  source: "central_file",
  locations: $zones,
  options: {playlist: [$item]}
}')
...
print_step "Broadcast running. Waiting ${PAUSE_SECONDS}s before stopping..."
sleep "$PAUSE_SECONDS"

print_step "Stopping broadcast..."
stop_broadcast "central_file_test"
ACTIVE_SESSION=0
