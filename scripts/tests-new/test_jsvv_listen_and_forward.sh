...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help)
        cat <<USAGE
Usage: $SCRIPT_NAME [--zones "101,102"] [--timeout SECONDS]
USAGE
        exit 0
        ;;
      *)
...
print_step "Dispatching synthetic JSVV request..."
JSVV_EVENT=$(jq -n \
  --argjson zones "$ZONES_JSON" \
  '{payload:{command:"REMOTE_VOICE", params:{zones:$zones}, priority:"P2"}}')
api_post "jsvv/events" "$JSVV_EVENT" >/dev/null
ACTIVE_SESSION=1
...
SESSION_JSON=""
while true; do
  SESSION_JSON=$(api_get "live-broadcast/status")
  STATUS_STATE=$(echo "$SESSION_JSON" | jq -r '.session.status // empty')
...
SOURCE=$(echo "$SESSION_JSON" | jq -r '.session.source // empty')
if [[ "$SOURCE" != "jsvv" ]]; then
  abort "Unexpected broadcast source: $SOURCE"
fi

SESSION_ZONES=$(echo "$SESSION_JSON" | jq '.session.applied.zones // .session.zones // []')
...
# ensure reported zones are subset of requested zones
